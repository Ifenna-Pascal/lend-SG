      ...
      <div className=" max-w-[2000px] mx-auto py-[1rem] md:py-[4px] xl:py-[4px]">
        ...
        <div className="max-w-[2000px]mx-auto mt-4 mb-[3rem] md:mt-10 px-8 md:px-10">
          {faqData.map((faq, index) => (
            <div
              key={index}
              className="mb-2 border border-[#01F8AF]  rounded-md"
            >
              <div
                onClick={() => toggleAnswer(index)}
                className="cursor-pointer  p-3 md:p-6  flex justify-between items-center "
              >
                <div className="font-bold text-[#FFFFFF] font-sora text-[10px] md:text-[20px] lg:text-[24px]">
                  {faq.question}
                </div>
                <div className="">
                  {activeIndex === index ? (
              ...
              {activeIndex === index && (
                <div className=" px-4  text-[#FFFFFF] font-urbanist font-light text-[8px] tracking-wide leading-[13.10px] mb-4  md:leading-relaxed w-[90%] md:text-[14px] lg:text-[16px]">
                  {faq.answer}
                </div>
              )}
            </div>
